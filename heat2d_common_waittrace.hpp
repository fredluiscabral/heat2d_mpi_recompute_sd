...
struct Solver {
    ...
    explicit Solver(const Config& c, bool adaptive_) : cfg(c), topo(build_topology(MPI_COMM_WORLD)), adaptive(adaptive_) {
        ...
        nx = bx.n; ny = by.n;
        wait_events.reserve(static_cast<size_t>(cfg.steps) * 2u);
        if (nx < 2 || ny < 3) {
    ...
    void acquire_up(RemoteSlot& cur, RemoteSlot* prev, int step) {
        ...
        wait_events.push_back(WaitEvent{step, 0, w});
        copy_vec_to_ghost(u,0,cur.halo_recv_up);
    ...
    void acquire_down(RemoteSlot& cur, RemoteSlot* prev, int step) {
        ...
        wait_events.push_back(WaitEvent{step, 1, w});
        copy_vec_to_ghost(u,ny+1,cur.halo_recv_down);
    ...
    void run() {
        ...
        // TRACE dinamico:
        // apenas amostragem local durante a ROI; nenhuma comunicacao nova.
        constexpr int TRACE_EVERY = 100;
        const int trace_n_samples =
            (cfg.steps + TRACE_EVERY - 1) / TRACE_EVERY;

        std::vector<double> trace_elapsed;
        std::vector<int> trace_steps;

        trace_elapsed.reserve(
            static_cast<std::size_t>(trace_n_samples));
        trace_steps.reserve(
            static_cast<std::size_t>(trace_n_samples));
        ...
        for (int step=0; step<cfg.steps; ++step) {
            ...
            const int trace_completed_step = step + 1;

            if ((trace_completed_step % TRACE_EVERY) == 0 ||
                trace_completed_step == cfg.steps) {

                trace_steps.push_back(trace_completed_step);

                trace_elapsed.push_back(
                    MPI_Wtime() - t0);
            }
        }
        ...
        // --------------------------------------------------------
        // Tudo abaixo ocorre depois da medida de local_elapsed.
        // Portanto os MPI_Gather nao entram no makespan medido.
        // --------------------------------------------------------
        if (static_cast<int>(trace_elapsed.size()) !=
            trace_n_samples) {
            throw std::runtime_error(
                "numero inesperado de amostras TRACE");
        }

        std::vector<double> trace_all;
        std::vector<int> trace_node_index;
        std::vector<int> trace_local_rank;

        if (topo.world_rank == 0) {
            trace_all.resize(
                static_cast<std::size_t>(topo.world_size) *
                static_cast<std::size_t>(trace_n_samples));

            trace_node_index.resize(
                static_cast<std::size_t>(topo.world_size));

            trace_local_rank.resize(
                static_cast<std::size_t>(topo.world_size));
        }
        ...
        MPI_Gather(
            ...
            trace_n_samples,
            MPI_DOUBLE,
            topo.world_rank == 0 ? trace_all.data() : nullptr,
            trace_n_samples,
            MPI_DOUBLE,
            0,
            world);
        ...
        // --------------------------------------------------------
        // Reconstrucao da trajetoria da criticidade.
        // --------------------------------------------------------
        if (topo.world_rank == 0) {

            for (int k = 0; k < trace_n_samples; ++k) {
                ...
                for (int r = 0; r < topo.world_size; ++r) {
                    ...
                    if (tr >
                        ...
                        node_max[
                            static_cast<std::size_t>(n)] = tr;

                        node_arg_local[
                            static_cast<std::size_t>(n)] = lr;
                    }
                }
        ...
        // --------------------------------------------------------
        // WAIT TRACE: agrega esperas reais nas mesmas janelas
        // temporais do TRACE. Tudo ocorre depois de local_elapsed.
        // --------------------------------------------------------
        const int wait_nslots = 2 * trace_n_samples;

        std::vector<long long> wait_bin_count(
            static_cast<std::size_t>(wait_nslots), 0);

        std::vector<double> wait_bin_sum(
            static_cast<std::size_t>(wait_nslots), 0.0);

        std::vector<double> wait_bin_max(
            static_cast<std::size_t>(wait_nslots), 0.0);

        for (const auto& e : wait_events) {
            int k = e.step / TRACE_EVERY;

            if (k < 0) k = 0;
            if (k >= trace_n_samples)
                k = trace_n_samples - 1;

            const int idx = 2 * k + e.direction;

            wait_bin_count[
                static_cast<std::size_t>(idx)]++;

            wait_bin_sum[
                static_cast<std::size_t>(idx)] += e.wait_s;
        ...
        std::vector<long long> wait_all_count;
        std::vector<double> wait_all_sum;
        std::vector<double> wait_all_max;

        if (topo.world_rank == 0) {
            const std::size_t total =
                static_cast<std::size_t>(topo.world_size) *
                static_cast<std::size_t>(wait_nslots);

            wait_all_count.resize(total);
            wait_all_sum.resize(total);
            wait_all_max.resize(total);
        }
