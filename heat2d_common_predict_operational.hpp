...
// PREDICT operacional MPI para a equacao do calor 2D (FTCS).
//
// Problema multimodal:
//   u0 = sin(pi x) sin(pi y) + 0.25 sin(3 pi x) sin(2 pi y)
//
// Quando um halo inter-no ainda nao chegou:
//   1) tenta construir Uhat^n = 2 U^(n-1) - U^(n-2)
//      usando SOMENTE halos MPI reais ja recebidos;
//   2) aplica o criterio de admissibilidade validado no diagnostico:
//        ry * (P_hat_safe) <= eta * B_hat + G_FTCS
//   3) se admissivel, usa imediatamente o halo predito e NAO bloqueia;
//   4) a mensagem MPI real continua em voo e e validada mais tarde.
//
// Nao ha RECOMPUTE nesta variante. O objetivo e comparar diretamente:
//   baseline: READ / WAIT
//   predict : READ / PREDICT / WAIT
//
// --enable-predict 0 desliga todo o trafego de suporte e fornece o baseline
// multimodal no MESMO executavel e com a MESMA discretizacao.
//
// Importante: esta e a primeira variante operacional. O bound foi validado
// originalmente para predicoes isoladas (D=0). Por isso registramos, ao final,
// o erro global da EDP e validamos cada halo predito contra a mensagem real.
// Isso permite medir simultaneamente desempenho e impacto numerico.
...
struct Config {
    ...
    std::string policy = "wait"; // sem RECOMPUTE nesta variante
...
inline Config parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nx") c.nx_global = std::stoi(need(i));
        ...
        else if (a == "--enable-predict") c.enable_predict = (std::stoi(need(i)) != 0);
            ...
            if (true) {
                std::cout << "Opções: --nx N --ny N --steps N --alpha A --cfl F\n"
                          << "         --policy wait --enable-predict 0|1 --beta B\n"
    ...
    if (c.policy != "wait")
        throw std::runtime_error("predict_operational exige --policy wait");
    if (!(c.eta > 0.0))
...
struct Stats {
    ...
    // PREDICT operacional.
    long long predict_attempts = 0;
    long long predicts = 0;
    long long predict_rejected = 0;
    long long predict_unavailable = 0;
    long long predict_no_history = 0;
    long long predict_validations = 0;
    long long predict_validation_violations = 0;
    double predict_linf_sum = 0.0;
    double predict_linf_max = 0.0;
    double predict_max_actual_ratio = 0.0;

    long long admiss_tests = 0;
    long long admiss_accepted = 0;
    long long admiss_actual_safe = 0;          // criterio efetivo: eta*B + G_FTCS
    long long admiss_actual_method_safe = 0;   // criterio estrito antigo: eta*B
    long long admiss_false_accepts = 0;
    long long admiss_false_rejects = 0;
    long long admiss_accepted_roundoff_only = 0;
    ...
    double admiss_max_ftcs_guard = 0.0;
...
struct RemoteSlot {
    ...
    // Estado tardio de uma predicao operacional.
    std::vector<double> predicted_up, predicted_down;
    std::vector<double> predict_allowed_up, predict_allowed_down;
    bool used_predict_up = false, used_predict_down = false;
    bool predict_validated_up = false, predict_validated_down = false;
    bool real_recorded_up = false, real_recorded_down = false;
    ...
    void resize(int nx) {
        ...
        predicted_up.assign(nx, 0.0); predicted_down.assign(nx, 0.0);
        predict_allowed_up.assign(nx, 0.0); predict_allowed_down.assign(nx, 0.0);
    }
};
...
struct Solver {
    ...
    std::vector<double> op_allowed_up, op_allowed_down;
    ...
    explicit Solver(const Config& c, bool adaptive_) : cfg(c), topo(build_topology(MPI_COMM_WORLD)), adaptive(adaptive_) {
        ...
        if (std::abs(rx - ry) > 1.0e-12 * rscale) {
            if (topo.world_rank == 0)
                std::cerr << "ERRO: predict_operational requer rx == ry (malha isotropica).\n";
            MPI_Abort(world, 6);
        }
        if (cfg.policy != "wait") {
            if (topo.world_rank == 0)
                std::cerr << "ERRO: predict_operational exige --policy wait.\n";
            MPI_Abort(world, 7);
        }
        ...
        op_allowed_up.assign(nx, 0.0);
        op_allowed_down.assign(nx, 0.0);
    ...
    void post_remote_halo(RemoteSlot& s, int step) {
        s.step = step;
        s.rec_up = s.rec_down = false;
        s.used_predict_up = s.used_predict_down = false;
        s.predict_validated_up = s.predict_validated_down = false;
        s.real_recorded_up = s.real_recorded_down = false;
        if (topo.up != MPI_PROC_NULL) {
            ...
            if (adaptive && cfg.enable_predict)
    ...
    void post_support(RemoteSlot& s) {
        if (!adaptive || !cfg.enable_predict) return;
        if (topo.up != MPI_PROC_NULL) {
    ...
    bool halo_real_ready(RemoteSlot& s, bool up) {
        MPI_Request& req = up ? s.halo_rr_up : s.halo_rr_down;
        if (req == MPI_REQUEST_NULL) return true;
        int ready = 0;
        MPI_Test(&req, &ready, MPI_STATUS_IGNORE);
        return ready != 0;
    ...
    double ftcs_round_guard_current(bool up,
                                    int j,
                                    double predicted_remote) const {
        // Bound absoluto para o arredondamento de UMA atualizacao FTCS
        // no ponto da fronteira consumidora, usando apenas dados disponiveis
        // antes da chegada do halo real.
        //
        // A implementacao avalia:
        //   c + rx*(l - 2c + r) + ry*(remote - 2c + inner)
        //
        // A multiplicacao por 2 e exata em binario na faixa normal; contamos
        // oito operacoes arredondadas: quatro +/-, dois produtos por r e
        // duas somas finais. O bound Higham usa a soma das magnitudes dos
        // termos que alimentam essa expressao.
        const int i = up ? 1 : ny;
        const int ii = up ? 2 : ny - 1;
        const int jj = j + 1; // j do halo e 0-based; Grid usa 1..nx.

        const double c = u(i, jj);
        const double l = u(i, jj - 1);
        const double r = u(i, jj + 1);
        const double inner = u(ii, jj);

        const double scale =
            std::abs(c)
            + rx * (std::abs(l) + 2.0 * std::abs(c) + std::abs(r))
            + ry * (std::abs(predicted_remote)
                    + 2.0 * std::abs(c) + std::abs(inner));

        return gamma_n(8) * scale;
    }
    ...
    AdmissEval evaluate_admissibility(bool up, int step) {
        ...
        // Historico de PREDICT usa apenas halos MPI REAIS. Se algum halo
        // historico ou suporte ainda estiver em voo, nao bloqueamos.
        if (!halo_real_ready(*s0, up) ||
            !halo_real_ready(*s1, up) ||
            !halo_real_ready(*s2, up) ||
            !support_ready(*s0, up) ||
            !support_ready(*s1, up) ||
            !support_ready(*s2, up))
            return out;
        ...
        out.ftcs_guard.assign(static_cast<std::size_t>(nx), 0.0);
        ...
        for (int j = 0; j < nx; ++j) {
            ...
            // O halo predito e exatamente o valor que seria consumido pela
            // atualizacao FTCS se PREDICT fosse aceito. Portanto G_FTCS pode
            // ser calculado online, sem qualquer informacao do halo real.
            const auto& pred = up ? pred_work_up : pred_work_down;
            const double Gftcs =
                ftcs_round_guard_current(up, j, pred[static_cast<std::size_t>(j)]);

            const double method_allowed = cfg.eta * B;
            const double allowed = method_allowed + Gftcs;
            ...
            out.ftcs_guard[static_cast<std::size_t>(j)] = Gftcs;
            ...
            out.max_ftcs_guard = std::max(out.max_ftcs_guard, Gftcs);
    ...
    bool evaluate_operational_admissibility(bool up,
                                            int step,
                                            std::vector<double>& allowed_out,
                                            bool& available_out) {
        available_out = false;
        ...
        for (int j = 0; j < nx; ++j) {
            ...
            const double allowed = cfg.eta * B + Gftcs;
            allowed_out[static_cast<std::size_t>(j)] = allowed;

            if (!(ry * Phat_safe <= allowed))
                accepted = false;
        }
    ...
    void validate_admissibility(const AdmissEval& ev,
        ...
        stats.admiss_max_ftcs_guard =
            std::max(stats.admiss_max_ftcs_guard, ev.max_ftcs_guard);

        bool actual_safe = true;
        bool actual_method_safe = true;
        ...
        for (int j = 0; j < nx; ++j) {
            ...
            const double method_allowed = cfg.eta * ev.budget[k];
            const double allowed = method_allowed + ev.ftcs_guard[k];
            ...
            if (!(ry * actual <= method_allowed)) actual_method_safe = false;
        ...
        if (actual_method_safe) stats.admiss_actual_method_safe++;
        if (ev.accepted && !actual_safe) stats.admiss_false_accepts++;
        if (!ev.accepted && actual_safe) stats.admiss_false_rejects++;
        if (ev.accepted && actual_safe && !actual_method_safe)
            stats.admiss_accepted_roundoff_only++;
    ...
    void validate_operational_prediction(RemoteSlot& s, bool up) {
        const bool used = up ? s.used_predict_up : s.used_predict_down;
        bool& validated = up ? s.predict_validated_up : s.predict_validated_down;
        if (!used || validated) return;

        MPI_Request& req = up ? s.halo_rr_up : s.halo_rr_down;
        if (req != MPI_REQUEST_NULL) return;

        const auto& pred = up ? s.predicted_up : s.predicted_down;
        const auto& real = up ? s.halo_recv_up : s.halo_recv_down;
        const auto& allowed = up ? s.predict_allowed_up : s.predict_allowed_down;
        ...
        for (int j = 0; j < nx; ++j) {
            const std::size_t k = static_cast<std::size_t>(j);
            const double actual = std::abs(pred[k] - real[k]);
            linf = std::max(linf, actual);

            double ratio = 0.0;
            if (allowed[k] > 0.0)
                ratio = (ry * actual) / allowed[k];
            else if (actual > 0.0)
                ratio = std::numeric_limits<double>::infinity();
            max_ratio = std::max(max_ratio, ratio);

            const double measurement_tol =
                ry * gamma_n(1) * (std::abs(pred[k]) + std::abs(real[k]));
            if (ry * actual > allowed[k] + measurement_tol)
                violation = true;
        }

        stats.predict_validations++;
        stats.predict_linf_sum += linf;
        stats.predict_linf_max = std::max(stats.predict_linf_max, linf);
        stats.predict_max_actual_ratio =
            std::max(stats.predict_max_actual_ratio, max_ratio);
        if (violation) stats.predict_validation_violations++;

        validated = true;
    }

    void harvest_real_history(bool up, int upto_step) {
        if (upto_step < 0) return;

        int& n1 = up ? pred_step_n1_up : pred_step_n1_down;
        while (n1 < upto_step) {
            const int next = n1 + 1;
            RemoteSlot* s = slot_at_step(next);
            if (!s) return;

            if (!halo_real_ready(*s, up)) return;

            validate_operational_prediction(*s, up);

            bool& recorded = up ? s->real_recorded_up : s->real_recorded_down;
            if (!recorded) {
                if (up)
                    record_real_up(next, s->halo_recv_up);
                else
                    record_real_down(next, s->halo_recv_down);
                recorded = true;
            } else {
                // Se ja foi registrado, n1 obrigatoriamente deveria ter avancado.
                // Esta guarda evita um loop infinito em caso de estado inconsistente.
                return;
            }
        }
    }

    void complete_slot_operational(RemoteSlot& s, bool count_cleanup) {
        if (s.step < 0) return;

        wait_req(s.halo_rr_up, stats, count_cleanup);
        wait_req(s.halo_rr_down, stats, count_cleanup);

        validate_operational_prediction(s, true);
        validate_operational_prediction(s, false);

        // Fecha eventuais lacunas do historico antes de reciclar o slot.
        if (topo.up != MPI_PROC_NULL)
            harvest_real_history(true, s.step);
        if (topo.down != MPI_PROC_NULL)
            harvest_real_history(false, s.step);

        // RECOMPUTE nao e usado aqui, mas mantemos a validacao estrutural.
        validate_if_needed(s, stats, cfg.validate_late);

        wait_req(s.sup_rr_up, stats, count_cleanup);
        wait_req(s.sup_rr_down, stats, count_cleanup);
        wait_req(s.halo_sr_up, stats, count_cleanup);
        wait_req(s.halo_sr_down, stats, count_cleanup);
        wait_req(s.sup_sr_up, stats, count_cleanup);
        wait_req(s.sup_sr_down, stats, count_cleanup);
        s.step = -1;
    }

    void store_prediction(RemoteSlot& cur,
                          bool up,
                          const std::vector<double>& predicted,
                          const std::vector<double>& allowed_src) {
        auto& dst = up ? cur.predicted_up : cur.predicted_down;
        auto& allowed = up ? cur.predict_allowed_up : cur.predict_allowed_down;
        bool& used = up ? cur.used_predict_up : cur.used_predict_down;
        bool& validated = up ? cur.predict_validated_up : cur.predict_validated_down;
    ...
    void acquire_up(RemoteSlot& cur, int step) {
        ...
        // Tenta incorporar mensagens reais atrasadas dos passos anteriores.
        harvest_real_history(true, step - 1);
        ...
        if (ready) {
            copy_vec_to_ghost(u, 0, cur.halo_recv_up);
            stats.reads++;
            harvest_real_history(true, step);
            return;
        }

        if (cfg.enable_predict) {
            stats.predict_attempts++;
            const bool pred_ok = make_predict_up(step);
            if (!pred_ok) {
                stats.predict_no_history++;
            } else {
                bool available = false;
                const bool accepted =
                    evaluate_operational_admissibility(true, step,
                                                      op_allowed_up, available);
                if (!available) {
                    stats.predict_unavailable++;
                } else if (accepted) {
                    store_prediction(cur, true, pred_work_up, op_allowed_up);
                    copy_vec_to_ghost(u, 0, pred_work_up);
                    stats.predicts++;
                    return;
                } else {
                    stats.predict_rejected++;
                }
            }
        }
    ...
    void acquire_down(RemoteSlot& cur, int step) {
        ...
        harvest_real_history(false, step - 1);
        ...
        if (ready) {
            copy_vec_to_ghost(u, ny + 1, cur.halo_recv_down);
            stats.reads++;
            harvest_real_history(false, step);
            return;
        }
        ...
        if (cfg.enable_predict) {
            ...
            if (!pred_ok) {
                ...
                bool available = false;
                const bool accepted =
                    evaluate_operational_admissibility(false, step,
                                                      op_allowed_down, available);
                if (!available) {
                    stats.predict_unavailable++;
                } else if (accepted) {
                    store_prediction(cur, false, pred_work_down, op_allowed_down);
                    copy_vec_to_ghost(u, ny + 1, pred_work_down);
                    stats.predicts++;
                    return;
                } else {
                    stats.predict_rejected++;
    ...
    std::string variant_name() const {
        return cfg.enable_predict ? "predict_operational" : "predict_baseline";
    }
    ...
    void run() {
        if (topo.world_rank==0) {
            ...
            std::cout << std::setprecision(17)
                      ...
                      << " enable_predict=" << (cfg.enable_predict?1:0)
        ...
        for (int step=0; step<cfg.steps; ++step) {
            ...
            if (cur.step >= 0) complete_slot_operational(cur, true);
            ...
            // 5) Se PREDICT estiver ativo, envia o suporte do criterio de admissibilidade.
            post_support(cur);

            // 6) Trabalho que depende só de halos intra-nó.
            compute_side_columns();

            // 7) READ | PREDICT | WAIT apenas para os vizinhos inter-no.
            acquire_up(cur, step);
            acquire_down(cur, step);
        ...
        for (auto& s: slots) if (s.step>=0) complete_slot_operational(s,true);
        ...
        long long loc_predict[7] = {
            ...
            stats.predict_no_history,
            stats.predict_validations,
            stats.predict_validation_violations
        };
        long long glob_predict[7] = {0,0,0,0,0,0,0};
        MPI_Reduce(loc_predict, glob_predict, 7,
                   MPI_LONG_LONG_INT, MPI_SUM, 0, world);

        double loc_predict_diag[3] = {
            stats.predict_linf_sum,
            stats.predict_linf_max,
            stats.predict_max_actual_ratio
        };
        double glob_predict_sum = 0.0;
        double glob_predict_max[2] = {0.0, 0.0};
        MPI_Reduce(&loc_predict_diag[0], &glob_predict_sum, 1,
                   MPI_DOUBLE, MPI_SUM, 0, world);
        MPI_Reduce(&loc_predict_diag[1], &glob_predict_max[0], 2,
        ...
        if (topo.world_rank==0) {
            ...
            std::cout << std::setprecision(12)
                      << "PREDICT_OP"
                      << " enabled=" << (cfg.enable_predict?1:0)
                      << " attempts=" << glob_predict[0]
                      << " predicts=" << glob_predict[1]
                      << " predict_pct="
                      << (glob_predict[0] > 0
                          ? 100.0 * static_cast<double>(glob_predict[1])
                              / static_cast<double>(glob_predict[0])
                          : 0.0)
                      << " rejected=" << glob_predict[2]
                      << " unavailable=" << glob_predict[3]
                      << " no_history=" << glob_predict[4]
                      << " validations=" << glob_predict[5]
                      << " validation_violations=" << glob_predict[6]
                      << " mean_linf_error="
                      << (glob_predict[5] > 0
                          ? glob_predict_sum / static_cast<double>(glob_predict[5])
                          : 0.0)
                      << " max_linf_error=" << glob_predict_max[0]
                      << " max_actual_ratio=" << glob_predict_max[1]
                      << " history=real_halos_only"
                      << " safe_def=etaB_plus_ftcs_roundoff"
                      << "\n";
            ...
            std::cout << std::setprecision(12)
                      ...
                      << " waits=" << glob_counts[1]
                      << " predicts=" << glob_predict[1]
                      << " predict_attempts=" << glob_predict[0]
                      << " predict_rejected=" << glob_predict[2]
                      << " predict_unavailable=" << glob_predict[3]
                      << " predict_no_history=" << glob_predict[4]
                      << " predict_validation_violations=" << glob_predict[6]
